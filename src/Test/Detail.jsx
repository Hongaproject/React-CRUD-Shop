...
function Detail (props) {
    ...
    useEffect(() => { // 최근 본 상품을 넣기위해 사용함. 
        let get_local = localStorage.getItem("data"); // 변수 선언 후 할당을 해줍니다.
        console.log(get_local);

        if (get_local == null) {
           get_local = []; // 아무것도 없으면 빈배열 추가해줌.
         } else {
           get_local = JSON.parse(get_local); // 배열안에 내용이 있으면 JSON.parse변환시켜 get_local에 저장한다.
         ...
         get_local.push(props.product[id].title); // 상세페이지에서 접속을 하면 ()부분을 가져와 push로 넣어준다.
         get_local = new Set(get_local); // set함수를 사용해서 중복을 제거해준다. 이 부분은 배열에 추가한 내용이 있으면 또 추가를 안해준다는 뜻
         get_local = [...get_local]; // spread문법을 사용해서 내용을 복사해주고
         localStorage.setItem("data", JSON.stringify(get_local)); // get_local로 다시 넣어줍니다.
